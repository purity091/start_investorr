...
export const HackathonView: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-tajawal" dir="rtl">
      {/* Navbar */}
      <nav className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-[60] shadow-sm text-right">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-2">
             <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-black text-xl italic">S</div>
             <span className="text-xl font-black text-slate-900 tracking-tight">START INVESTOR</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
           <button className="px-5 py-2 text-sm font-black text-slate-700 hover:bg-slate-50 rounded-xl transition-all">تسجيل الدخول</button>
           <button className="px-5 py-2 bg-indigo-600 text-white text-sm font-black rounded-xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">بدء مشروع</button>
        </div>
      </nav>

      <div className="relative">
        {/* Sticky Secondary Nav for Tabs (appears on scroll - logic would be here, but using z-index and sticky for CSS-only feel) */}
        <div className="sticky top-16 z-50 bg-white border-b border-slate-200 shadow-md hidden lg:block h-0 overflow-visible">
           <div className="container mx-auto px-6 h-16 flex items-center bg-white border-b border-slate-200 translate-y-[-100%] hover:translate-y-0 transition-transform duration-300">
              <div className="flex gap-8">
                 {['overview', 'goals', 'rules', 'mechanism', 'outcomes', 'schedule'].map((id) => (
                    <button 
                       key={id}
                       onClick={() => setActiveTab(id as any)}
                       className={`text-sm font-black transition-all ${activeTab === id ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900'}`}
                    >
                       {id === 'overview' ? 'نظرة عامة' : id === 'goals' ? 'الأهداف' : id === 'rules' ? 'الضوابط' : id === 'mechanism' ? 'الآلية' : id === 'outcomes' ? 'المخرجات' : 'الجدول'}
                    </button>
                 ))}
              </div>
           </div>
                    ...
                    <div className="flex overflow-x-auto no-scrollbar gap-8">
                       {[
                         ...
                         <button
                           ...
                           className={`py-5 text-sm font-black transition-all whitespace-nowrap border-b-4 ${
                             activeTab === tab.id 
