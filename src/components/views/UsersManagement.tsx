...
const names = ['أحمد محمد', 'سارة خالد', 'خالد عبد الله', 'نورة العتيبي', 'عمر الفاروق', 'فهد الدوسري', 'ريم السالم'];

const MOCK_USERS: ManagedUser[] = Array.from({ length: 36 }).map((_, index) => ({
  id: `USR-${1000 + index}`,
  name: `${names[index % names.length]}${index > names.length ? ` ${index}` : ''}`,
  email: `user${index + 1}@example.com`,
  role: (['admin', 'manager', 'user', 'user', 'user'][index % 5]) as UserRole,
  status: (['active', 'active', 'pending', 'suspended', 'active'][index % 5]) as UserStatus,
  registrationDate: new Date(Date.now() - (index + 2) * 86400000).toISOString().split('T')[0],
}));
...
export const UsersManagement: React.FC = () => {
  ...
  const toggleStatus = (id: string, status: UserStatus) => {
    setUsers((prev) =>
      prev.map((user) => (user.id === id ? { ...user, status: status === 'active' ? 'suspended' : 'active' } : user)),
    );
  };
